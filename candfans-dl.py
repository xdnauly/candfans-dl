...
class FollowedUser(BaseModel):
    "user that you are followling"
    user_id: int
    user_code: str
    username: str


class ChangedAPIError(Exception):
    "Raised when candfans chand their API"
    pass
...
def get_user(user_code: str) -> Dict:
    "get follow user"
    return httpx.get(
        url=f"{URL}/user/get-users?user_code={user_code}", headers=header()
    ).json()["data"]["user"]
...
def get_subscriptions(userinfo: Tuple[int, str]) -> List[Dict[str, Any]]:
    ...
    for i in range(1, math.ceil(follow_cnt / 10) + 1):
        ...
        sub = httpx.get(
            url=f"{URL}/user/get-follow/{user_id}",
            headers=header(),
            params={
                "page": i,
            },
    ...
    for user in selected_models:
        ...
        for img_url in img_urls:
            if img_url != "":
                ...
                tasks.append(
                    async_download_file(f"{CONTENT_URL}{img_url}",
                                        file_name,
                                        "info")
                )
        ...
        for file in itertools.chain(
            pathlib.Path("profiles/" + PROFILE + "/photos").iterdir(),
...
def get_all_photos(user_id: int, user_code: str) -> Tuple[int, int]:
    ...
    with tqdm(total=post_cnt, unit="posts", desc="posts") as pbar:
        for i in range(1, math.ceil(post_cnt / 20) + 1):
            posts = httpx.get(
                url=f"{URL}/contents/get-timeline",
                headers=header(),
                params={"user_id": user_id, "page": i},
            ).json()["data"]
